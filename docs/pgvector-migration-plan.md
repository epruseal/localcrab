...
- **`add_texts`/`upsert_texts`:** `vectors = ef(texts)` → `(node_id, pack_id, embedding, document, metadata)` INSERT. **upsert 는 `node_id` 기준 DELETE-후-INSERT**(구현 확정) — vec0 가상테이블은 `INSERT OR REPLACE`·`ON CONFLICT` 를 지원하지 않음을 실측 확인.
