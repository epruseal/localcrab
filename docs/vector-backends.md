...
조건부 쓰기도 트랜잭션도 없고 프로세스 간 잠금이 공유 락이라 선검사와
프로세스 내 락까지가 한계다. 공유 락은 보유자끼리 서로를 막지 않으므로 아무것도 직렬화하지
않는다. #140 이 그 잠금을 `ChromaStore` 인스턴스 수명으로 옮겨 MCP 뿐 아니라 REST 앱과
CLI 도 잡게 됐지만, 그 변화는 배타 요구자(오프라인 로더와 마이그레이션)를 상대로 한
배제를 넓힌 것이지 공유 보유자끼리의 직렬화를 만들지는 않는다. 프로세스 간 직렬화는 종전에도 이 계층이 제공하지 않았고 지금도
...
`--apply` 가 있어야 한다. 백업 경로를 `--backup-to <path>` 로 주면 복구 대상 행이
있을 때 그 게시가 성공한 뒤에만 DB 를 커밋하고(게시 실패는 트랜잭션 전체를
되돌린다), 대상 행이 0 건이면(멱등 재실행 등) 백업할 내용이 없어 파일을 만들지
않고 그대로 커밋한다. 백업을 명시적으로 생략하려면(`--skip-backup`)도 이 게이트
없이 바로 커밋한다. `--rollback-from`
...
충돌하는 `node_id` 로 써 보는 것만으로 남의 팩 이름을 알아낼 수 있다. `pack/write_gate.py` 의
`identity_reject_message` 가 graph 층에 세운 불변식과 같다. 호출자가 되받는 것은 자기가 낸 id 와
그 개수뿐이다. 한 배치 안의 충돌은 예외인데, 두 값 모두 호출자가 방금 넘긴 것이라 새로 드러나는
것이 없다.
