...
**러너가 확인하지 않는 것 셋.** 3단계의 수동 `opencrab init` 병기 검증은 하지
않는다 — 그 경로는 위 정본 명령으로 별도의 빈 데이터 루트에서 직접 확인한다.
서버가 stderr 로 내는 생성 공지도 관측하지 않는다 — 클라이언트가 MCP 서버
자식의 stderr 를 호출자에게 넘겨주지 않기 때문이며, 관측 가능한 증거는 생성된
파일이다. 그리고 **데이터 루트 생성 자체는 러너가 한다**, 클라이언트가 아니라.

마지막 항목은 실측으로 확인한 제약이다: 신선한 `HOME` 에서 데이터 루트가 없으면
클라이언트의 최초 stdio 기동이 `failed to start server ... Connection closed` 로
실패한다. 서버는 자기 데이터 디렉터리를 만들지 않고(동봉 README 가 명시한다),
클라이언트도 최초 기동 전에는 만들어 두지 않는다. 따라서 러너는 빈 디렉터리까지만
준비하고, **그 안에 스토어가 수동 init 없이 생기는가**를 관측한다 — 그것이 자동
부트스트랩 opt-in 의 계약이다.
