...
설치, 발견, 데이터 루트가 빈 것 확인, embedded turn, 자동 부트스트랩으로
`opencrab.db` 가 생겼는지 확인, 사후 독립 조회, 증거 판정. **3단계의 수동
`opencrab init` 병기 검증은 러너가 하지 않는다** — 그 경로는 위 정본 명령으로
별도의 빈 데이터 루트에서 직접 확인한다.
