...
1. **HTTP**: `http_app.py` 의 POST 핸들러에서 **인증 통과 이후, 각 dispatch(배치 루프와 단일 요청) 이전에 반환하는 모든 지점**을 경로별로 조사한다.
2. **stdio**: `server.py` 의 `_handle_raw` **진입부터** `handle_request` 까지를 조사한다. 파싱 거부가 `handle_request` 바깥에 있어 `handle_request` 만 보면 놓친다.
3. **공용**: `handle_request` 안에서 era 분기보다 앞서 반환하는 지점과 legacy-disabled 게이트.
4. **분류**: 위에서 나온 반환 중 **legacy 또는 era 미상 요청이 도달할 수 있는 거부만** 계측 후보다. era 가 modern 으로 확정된 요청의 거부(modern 알림의 바디 검증 거부 등)는 후보가 아니다.
...
| 7 | **JSON 파싱 실패 거부** — HTTP 400 + -32700, stdio 는 `_handle_raw` 의 동일 -32700 | 활성 구성을 **조회하지 않는다** | **전 구성**. 다른 여섯 지점 **전부보다 앞선다**. 귀속은 **era 미상** — `method` 도 `_meta` 도 읽을 수 없다 |
...
- 계수 지점: **계측 구현 시점에 위 절차로 재도출한 전체 집합**. 표의 일곱 행은 오늘 시점 스냅샷일 뿐이며, 고정 목록에 성공 조건을 매면 누락된 지점이 있어도 "거부 0" 으로 판정된다. 재도출한 지점들은 서로 배타적이므로 요청당 중복 계수는 없다.
