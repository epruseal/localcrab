...
| 대화 reingest hook (`lc_call`) | `initialize` 없이 legacy `tools/call` POST, `_meta`·MCP 헤더 없음 | **저장소 밖 · 미관측** (hooks 번들은 이 저장소에 없다) | 가능 — 요청에 `_meta` 와 표준 헤더를 추가하면 된다 | 운영자 |
...
modern-only 구성에서 legacy 트래픽이 거부되는 지점은 **다섯 곳**이고 그중 셋은 `handle_request` 에 도달하기 전이다.

1. `handle_request` 의 legacy-disabled 게이트 — 단일 요청, 오류 봉투 반환. **로그를 남기지 않는다.**
2. HTTP 계층의 **배치 배열 거부** — 원소 처리 전에 400 + -32600.
3. HTTP 계층의 **legacy 형상 알림 거부** — 빈 바디 400. **응답 본문이 없어 본문 식별로는 잡히지 않는다.**
4. HTTP 계층의 **비객체 scalar 바디 거부** — 400 + -32600.
5. **stdio** — HTTP 계층 자체가 없다.
...
- (ii) **엣지에서 계측을 지정한다.** 서버 코드를 그대로 두는 대신 프록시·게이트웨이에서 거부를 분리한다. 다만 3번(빈 바디)은 응답 본문으로 식별할 수 없고 5번(stdio)은 엣지를 지나지 않으므로, 이 갈래는 그 두 표면을 구조적으로 관측하지 못한다.
...
- 계수 지점: 위 다섯 곳 전부.
- 계수 단위: 요청 1건 = 1. **배치 배열은 원소별로 계수**하고 **알림도 포함**한다. 형상 오류로 거부된 요청도 legacy 표면 사용이므로 계수한다.
...
C1~C4 가 모두 충족되고 제거가 실행되면 이 문서는 제거 PR 의 근거 기록이 되며, `docs/mcp-protocol-compat.md` 에서 legacy 절이 삭제된다. 그 전까지 이 문서는 **미결 상태**다.
