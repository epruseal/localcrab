...
요청이 거부되는 지점은 **여섯 곳**이고, 서로 배타적이다(한 요청이 두 곳에서 세어지지 않는다). 넷은 `handle_request` 에 도달하기 전이다. 1·2 번은 modern-only 구성에서 발동하고, 3~5 번도 modern-only 구성의 거부이며, 6 번만 **구성과 무관하게** 발동한다.

1. `handle_request` 의 **`method` 부재·비문자열 거부** — era 분기보다 먼저 -32600 을 반환한다.
2. `handle_request` 의 **legacy-disabled 게이트** — 단일 요청, 오류 봉투 반환. **로그를 남기지 않는다.** stdio 와 HTTP 가 **공유**한다.
3. HTTP 계층의 **배치 배열 거부** — 원소 처리 전에 400 + -32600.
4. HTTP 계층의 **legacy 형상 알림 거부** — 빈 바디 400. **응답 본문이 없어 본문 식별로는 잡히지 않는다.**
5. HTTP 계층의 **비객체 scalar 바디 거부** — 400 + -32600.
6. HTTP 계층의 **헤더 검증 거부** — modern 버전 헤더가 붙은 legacy 바디는 -32020 + 400 이다. **구성 무관**으로 발동한다(활성 버전 구성을 조회하지 않고 빌드 내장 상수로 판정한다). 다섯 지점 어디에도 도달하지 않으며, **부분 이행 소비자의 신호**다 — 헤더는 modern 으로 바꿨는데 바디는 아직 legacy 인 클라이언트가 여기서만 드러난다.
...
- (ii) **엣지에서 계측을 지정한다.** 서버 코드를 그대로 두는 대신 프록시·게이트웨이에서 거부를 분리한다. 다만 4번(빈 바디 알림)은 응답 본문으로 식별할 수 없고 **stdio 트래픽은 엣지를 지나지 않아 2번의 stdio 몫이 통째로 누락**되므로, 이 갈래는 그 두 표면을 구조적으로 관측하지 못한다.
...
- 6번(헤더 검증 거부)의 귀속은 **원칙이 그대로 적용된다**: era 의 단일 출처는 요청 바디이고 헤더는 era 판정에 쓰지 않으므로(`docs/mcp-protocol-compat.md` 의 era 모델), 유효한 문자열 `method` 를 가진 `_meta` 없는 바디는 modern 헤더가 붙어도 **legacy** 다. `method` 가 없거나 비문자열인 변형만 era 미상이다. 미정 항목이 아니다.
