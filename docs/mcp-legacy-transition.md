...
| OpenClaw 호스트 (agent-plugin 번들) | `initialize` echo | `docs/agent-plugin-packaging.md` 의 호스트별 표 (**그 표가 정본이며 여기서 복제하지 않는다**) | 호스트가 2026-07-28 협상으로 전환해야 한다 | 소비자 측 |
...
modern-only 구성에서 요청이 거부되는 지점은 **다섯 곳**이고, 서로 배타적이다(한 요청이 두 곳에서 세어지지 않는다). 셋은 `handle_request` 에 도달하기 전이다.
...
- (i) **위 다섯 지점 전량에 로그 또는 카운터를 추가한다.** 서버 게이트 하나만으로는 부족하다 — 덮지 않은 지점이 있으면 실제 legacy 시도가 있었는데도 "거부 0" 으로 판정하게 된다. 조회식과 집계 범위를 함께 정의한다. 안 A 의 era 계수와 같은 축이므로 함께 도입하는 것이 자연스럽다.
...
- 계수 지점: 위 다섯 곳 전부. 서로 배타적이므로 요청당 중복 계수는 없다.
