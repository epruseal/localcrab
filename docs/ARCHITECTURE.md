...
- `openai` (권장): OpenAI 호환 임베딩 서버 백엔드. 기본 모델은 KURE-v1 (한국어 SOTA, 1024d). 컬렉션: `opencrab_vectors_kure`.
...
make_vector_store(settings)
  └─ EMBEDDING_BACKEND=openai
       └─ ChromaStore("opencrab_vectors_kure", ef=ResilientEmbeddingFunction)
            ├─ primary: LMStudioEmbeddingFunction (GPU, http://<lmstudio-host>:1234)
...
- `opencrab/config.py` — `embedding_backend`, `lmstudio_*`, `local_gguf_path` 등
