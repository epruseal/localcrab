...
9. [그래프/문서 노드 키 집합 정합성 (#317)](#9-그래프문서-노드-키-집합-정합성-317)
...
배타 획득 직후에는 `write.lock` 파일 자체에 진단용 보유자 레코드
(pid, 획득 시각, 있으면 purpose)를 남긴다(issue #352). 이 레코드는 순수
진단용이라 flock 의미론을 바꾸지 않는다 — 읽기와 쓰기 어느 쪽이
실패해도 로그만 남기고 삼키며 획득과 해제를 막지 않는다. 공유 획득
(`chroma.lock`의 다중 리더)에는 남기지 않고, 해제 시에도 지우지 않는다
(다음 배타 보유자가 그대로 덮어쓴다). 대기가 타임아웃되면 이 레코드를
읽어 안내 메시지에 보유자 정보를 붙인다 — 레코드가 없거나 헐었으면
"보유자 불명"으로 안내한다.
...
남는다. 한쪽 스토어에만 있는 노드는 그 방향의 쓰기가 실패했거나 예전 경로가 남긴
...
`None`이 아니면) 정규화 검사보다 먼저 검사해 치유 대상에서 제외하고
`property_error` 사유로 보고한다(`_classify_graph_only()`). PG JSONB
컬럼이 이미 디코드한 문자열 스칼라를 다시 파싱해 우연히 성공하는 값(예:
`'{}'`)도 이 검사로 걸러진다 -- 재파싱 성공이 그 컬럼이 실제로 객체를
담았다는 증거가 되지 않기 때문이다(#317, `opencrab/common/graph_identity.py`의
`decode_raw_properties()`).

**치유 방향**: 그래프 전용 노드는 문서 스토어로 역채움한다. 그래프가 정본이므로
문서가 그 뒤를 따른다. 문서 전용 노드는 `--promote-doc-only` 플래그를 줘야
그래프로 승격한다. 그래프가 정본인 이상 승격은 옵트인이다. 진단 시점에
`prepare_node()`로 승격 가능 여부를 미리 검증하고, 검증을 통과하지 못한 노드는
치유 대상에서 빼고 사유와 함께 보고한다. 적용 직전에는 문서 스토어의 현재
상태를 다시 조회해(`get_node_docs_by_id()`) 원본 소실(진단 이후 문서 삭제)과
신규 space 중복(진단 이후 다른 space에 같은 node_id 추가)을 감지한다 -- 이
재확인 없이 진단 시점 값을 그대로 신뢰하면 이미 삭제된 문서가 빈 속성 그래프
노드로 되살아날 수 있다. 재확인을 통과한 값에만 같은 검증(`prepare_node()`)을
...
실제로 행을 새로 썼는지, 그 행이 정상 형태로 저장돼 있는지, 기존 행이
있었다면 이번 실행이 그것을 갱신했는지는 보장하지 않는다 --
`upsert_node()`가 동일 digest의 기존 행을 발견하면 INSERT도 UPDATE도
실행하지 않고 idempotent로 반환할 수 있기 때문이다. 이 재해석 문제
자체(`upsert_node()`가 `_as_dict()`로 기존 행을 재해석해 충돌을 놓치는
경로)는 `#317`의 범위 밖이며 `#402`로 이관했다.

**적용과 백업**: 실제 쓰기는 `--apply`가 있어야 한다. 로컬 모드는 그 전에
`--backup-to <경로>`로 `opencrab/stores/backup.py`의 온라인 백업(#128)을 거치거나
`--skip-backup`을 명시해야 한다. `pg` 모드는 이 백업이 실제 PostgreSQL 데이터를
보호하지 못하므로 `--backup-to`를 줘도 `--skip-backup`을 함께 명시해야 적용을
받는다. 적용 직전에 스키마 상태를 다시 읽어 `target`이 아니면 거부한다.

**실행 기록**: `--apply` 실행은 JSON Lines 실행 기록을 남긴다. 기록 파일 쓰기가
실패해도 이미 끝난 스토어 쓰기는 되돌리지 않는다. 그 실패는 표준 오류로 알리고
0이 아닌 종료 코드로 끝난다.

```bash
python scripts/reconcile_doc_graph_nodes.py                                     # 진단만 (dry-run)
python scripts/reconcile_doc_graph_nodes.py --apply --backup-to /path/to/backup # 치유 적용
```
