...
`chroma/`, `docs/`처럼 디렉터리로 존재하는 대상은 심볼릭 링크를 따라가지
않고 링크 그 자체로 보존한다. 링크가 가리키는 실제 내용은 백업에 담기지
않는다.
...
이전 실행이 중단됐다는 뜻이다. 발행 직전 단계에서 실패했거나 발행
`rename` 자체가 실패한 경우에도 이 디렉터리가 남는다. 이름만으로 완료된
