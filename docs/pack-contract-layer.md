...
수치에는 **그 값을 낸 커밋**을 함께 적는다. 커밋이 없으면 중간 측정치를 기준선으로
인용하는 사고가 난다 — 실제로 났다(아래 주석).

| 모듈 | 커밋 | 총 | KILLED | 생존 | 생존의 성격 |
|---|---|---|---|---|---|
| `gates/score.py` | `0891447^` | 326 | 164 | 162 | 격자 도입 **전** |
| `gates/score.py` | `33a250c` | 326 | 317 | 9 | 전량 등가 증명(엣지 label 미사용 5 · s6 하한 도달불가 2 · `most_common(1)` 1 · `min(1.0)` 1) |
...
> 그래서 표에 커밋 열을 둔다 — 커밋을 적는 순간 중간값을 기준선으로 쓸 수 없다.
