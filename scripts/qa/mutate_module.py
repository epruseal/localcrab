...
**등가 측정의 함정 — 이 작업에서 세 번 반복했다(2026-08-05).**

등가 측정도 결국 테스트다. 입력이 **그 변이가 건드리는 축을 갈라야** 한다. 안 그러면
"차분 0" 이 나와도 그건 등가의 증거가 아니라 **입력이 빈약하다는 증거**다.

  뒤집힌 사례                       내가 쓴 입력            빠뜨린 입력
  ─────────────────────────────────────────────────────────────────────
  rsplit(".", 1) -> 2               chunks.jsonl            chunks.v1.jsonl
                                    (stem 의 점 1개)        (점 2개 -> int() 터짐)
  ev() 의 'contains' -> ''          라벨만 비교             properties 까지 비교
                                                            (source_label:'' 이 붙는다)
  {row.get('id')} -> {row.get('')}  id 가 없는 행           id 가 **있고** 다른 필드가
                                    (양쪽 다 None)          빠진 행 (id vs None 으로 갈림)

세 번 다 같은 모양이다 — **변이가 만지는 값이 입력에서 상수였다.** 등가를 주장하기 전에
"이 변이가 읽는 것이 무엇인가, 내 입력에서 그게 변하는가"를 먼저 답하라.
...
PACK_SUITES: dict[str, tuple[str, ...]] = {
    ...
    "opencrab/pack/load.py": ("tests/test_pack_load.py", "tests/test_delete_pack_journal.py"),
    "opencrab/pack/delete_journal.py": ("tests/test_delete_pack_journal.py",),
