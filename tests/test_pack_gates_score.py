...
격자 도입 전에는 생존 58 이었다. 남은 9 는 전부 **등가 증명이 붙어 있고**
`TestProvenEquivalences` 가 그 전제를 계약으로 건다 — 전제가 깨지면 거기서 빨간불이 난다.
...
class TestProvenEquivalences:
    ...
    def test_edge_label_is_never_read_by_the_rubric(self, tmp_path):
        ...
        두 줄에 가해진 변이 7종이 전부 생존한다 — 결함이 아니라 **도달 불가**다.
        생산자 계층이 label 을 요구하므로 줄 자체는 남긴다.

        이 단언이 깨지면 = 누가 label 을 채점에 쓰기 시작했다는 뜻이고, 그때는 위 7종이
