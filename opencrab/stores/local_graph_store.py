...
class LocalGraphStore(_SqliteConnMixin):
    ...
    def find_neighbors(
        ...
        while queue and len(results) < limit:
            ...
            if direction in ("out", "both"):
                self._expand(
                    cur, "out", current_id, current_depth,
                    visited, results, queue, limit, pack_set, include_unpackaged,
                )

            if direction in ("in", "both"):
                # direction="both"일 때 outgoing이 limit을 채운 경우에도 외부 while은
                # 다음 반복 시작 시에야 확인하므로, incoming 호출 전에 멈추지 않는다.
                # _expand 내부에서 remaining을 재계산해 슬롯 소진 시 쿼리 자체를 건너뛴다.
                self._expand(
                    cur, "in", current_id, current_depth,
                    visited, results, queue, limit, pack_set, include_unpackaged,
                )
    ...
    def _expand(
        ...
        """Expand one direction ("out" or "in") of find_neighbors' BFS by one hop.
        ...
        ``direction="out"``과 ``"in"``은 컬럼명/WHERE절/_edge_passes 인자 순서만
        다르고 로직은 동일하다 — 여기서 하나로 합친다.
        """
        ...
        if direction == "out":
            ...
            type_key, id_key = "to_type", "to_id"
        else:
            sql = (
                "SELECT from_type, from_id, relation, properties"
                " FROM graph_edges WHERE to_id=? LIMIT ?"
            )
            type_key, id_key = "from_type", "from_id"

        cur.execute(sql, (current_id, remaining))
        for row in cur.fetchall():
            ...
            if nid in visited:
                continue
            neighbor_props = self._fetch_node_props(cur, row[type_key], nid)
            if not neighbor_props:
                continue
            if pack_set is not None:
                neighbor_pass = _node_passes(neighbor_props, pack_set, include_unpackaged)
                if not neighbor_pass:
                    continue
                edge_props = parse_props(row["properties"])
                src_ok, dst_ok = (
                    (True, neighbor_pass) if direction == "out" else (neighbor_pass, True)
                )
                if not _edge_passes(edge_props, src_ok, dst_ok, pack_set):
                    continue
            visited.add(nid)
            results.append({
