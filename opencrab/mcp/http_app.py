...
def _resolve_token(cli_token: str | None = None, cli_token_file: str | None = None) -> str | None:
    """
    Resolve the bearer token, mirroring the localcrab auth-proxy convention.

    Precedence: explicit CLI token > LOCALCRAB_MCP_TOKEN env > token file
    (CLI ``--auth-token-file`` or LOCALCRAB_MCP_TOKEN_FILE env). Returns None
    when no source is configured, which leaves the instance unauthenticated.
    """
    if cli_token:
        return cli_token.strip()
    env_token = os.environ.get("LOCALCRAB_MCP_TOKEN", "").strip()
    if env_token:
        return env_token
    path = cli_token_file or os.environ.get("LOCALCRAB_MCP_TOKEN_FILE")
    if path:
