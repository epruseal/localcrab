...
**이 모듈은 임시 우회다.** 정체성이 팩으로 한정되지 않는다는 근본 결함(#197 —
`graph_nodes` PK 는 `(node_type,node_id)`, Kuzu 는 `node_id` 단독, 벡터 3종은
`node_id` 전역 유일)이 아직 해소되지 않았기 때문에, fork 는 원본과 같은 id 로
콘텐츠를 복사할 수 없다 — 같은 id 로 쓰면 그래프는 UPSERT 라 원본 행을 조용히
덮고, 벡터 삭제 술어에는 pack 조건이 없다. 그래서 fork 호출마다 하나의 salt 로
모든 콘텐츠 id 를 결정적으로 재매핑한다. **#197 이 해소되면 이 모듈과 그 호출부
(`opencrab/pack/fork.py`)를 통째로 지우면 된다** — 정체성이 팩으로 한정되는
순간 재매핑 자체가 필요 없어진다.
